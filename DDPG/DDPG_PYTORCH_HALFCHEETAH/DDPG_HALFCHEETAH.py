...
# Neural network for Q-function (critic)
...
class DDPGAgent:
    def __init__(self, state_dim, action_dim, actor_hidden_sizes, critic_hidden_sizes, 
                 actor_lr, critic_lr, gamma, tau, buffer_size, batch_size, activation=nn.ReLU):
        ...
        # Actor networks
        self.actor = Actor(state_dim, action_dim, actor_hidden_sizes, activation).to(device)
        self.target_actor = Actor(state_dim, action_dim, actor_hidden_sizes, activation).to(device)
        self.target_actor.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=actor_lr)

        # Critic networks
        self.critic = Critic(state_dim, action_dim, critic_hidden_sizes, activation).to(device)
        self.target_critic = Critic(state_dim, action_dim, critic_hidden_sizes, activation).to(device)
...
if __name__ == "__main__":
    ...
    while total_steps < args.max_steps:
        ...
        if total_steps % args.log_interval == 0:
            ...
            print(f"Steps: {total_steps}, Average Return: {avg_return:.2f}")
